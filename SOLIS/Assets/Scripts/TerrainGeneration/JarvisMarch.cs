...
public static class JarvisMarchAlgorithm
{
    public static List<Vector2> GetConvexHull(List<Vector2> points)
    {
        ...
        //Step 1. Find the Vector2 with the smallest x coordinate
        //If several have the same x coordinate, find the one with the smallest z
        Vector2 startVector2 = points[0];

        Vector2 startPos = startVector2;
        ...
        for (int i = 1; i < points.Count; i++)
        {
            ...
            {
                startVector2 = points[i];

                startPos = startVector2;
            }
        }

        //This Vector2 is always on the convex hull
        convexHull.Add(startVector2);

        points.Remove(startVector2);
